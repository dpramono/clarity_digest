...
def score_paper(paper, ref_files=None, key_overrides=None, rubric=None):
...
def main():
    ...
    digest_file = out_dir / "clarity_digest.md"
    ...
    scored = []
    for p in papers:
        res = score_paper(p, ref_files=ref_files, key_overrides=key_overrides, rubric=rubric)
        p.update(res)
    ...
    with open(digest_file, "w", encoding="utf-8") as f:
        f.write(f"# ClarityDigest - {dt.date.today().isoformat()}\n")
    ...
    # Kopie im Hauptverzeichnis aktualisieren für schnellen Zugriff
     with open("cardio_digest.md", "w", encoding="utf-8") as f:
