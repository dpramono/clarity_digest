...
# ---------------------------------------------------------------------------
# Config: Einlesen von config/gemini_clarity.cfg
# Bewusst relativ zum Skript-Speicherort (nicht zum Arbeitsverzeichnis) aufgeloest:
# Die Config gilt fuer ALLE Fachgebiete gemeinsam (API-Keys, Modellname) und soll
# daher immer neben gemini_clarity_digest.py liegen - unabhaengig davon, aus welchem
# Ordner das Script gestartet wird oder welches --specialty-dir gewaehlt ist.
# ---------------------------------------------------------------------------

SCRIPT_DIR = Path(__file__).resolve().parent
config_file = SCRIPT_DIR / "config" / "gemini_clarity.cfg"
...
if config_file.is_file():
    ...
    if config.has_section("GENERAL"):
        if config.get("GENERAL", "dry_run", fallback=""):
            os.environ["DRY_RUN_DEFAULT"] = config.get("GENERAL", "dry_run")
...
# Persistenter Test-Phase-Schalter: in config/gemini_clarity.cfg unter [GENERAL] dry_run = true/false
# setzbar, damit man waehrend der Testphase nicht bei jedem Aufruf --dry-run tippen muss.
# CLI-Flags --dry-run / --live ueberschreiben diesen Default pro Lauf (siehe main()).
DRY_RUN_DEFAULT = os.environ.get("DRY_RUN_DEFAULT", "false").strip().lower() in ("1", "true", "yes", "on")
...
def main():
    ...
    parser.add_argument("--key-findings", type=str, default="key_findings.json",
                         help="Pfad zur JSON-Datei mit manuellen Bottom-Line-Overrides je PMID "
                              "(relativ zu --specialty-dir, falls gesetzt).")
    parser.add_argument("--refs-dir", type=str, default="refs",
                         help="Pfad zum Ordner mit Referenz-PDFs (relativ zu --specialty-dir, falls gesetzt).")
    ...
    parser.add_argument("--specialty-dir", type=str, default=None,
                         help="Pfad zum Fachgebiets-Ordner (enthaelt refs/, params/, key_findings.json, "
                              "output/). Das Script wechselt vor der Verarbeitung dorthin, sodass ein "
                              "einziges gemini_clarity_digest.py fuer beliebig viele Fachgebiete "
                              "(Dermatologie, Innere Medizin, Pulmologie, ...) verwendet werden kann, "
                              "jedes mit eigenen Params/Refs/Output. Alle anderen --*-file-Pfade und "
                              "--refs-dir/--key-findings bleiben relativ dazu.")
    parser.add_argument("--dry-run", dest="dry_run", action="store_true", default=DRY_RUN_DEFAULT,
                         help="Testmodus: laedt und prueft nur alle Parameter-/Referenzdateien "
                              ...
                              "E-Utilities/Gemini. Aktueller Default (aus Config bzw. eingebaut): "
                              f"{DRY_RUN_DEFAULT}.")
    parser.add_argument("--live", dest="dry_run", action="store_false",
                         help="Erzwingt einen echten Lauf mit PubMed- und Gemini-Aufruf fuer diesen "
                              "Aufruf, auch wenn dry_run=true in der Config gesetzt ist.")
    args = parser.parse_args()

    if args.specialty_dir:
        target = Path(args.specialty_dir)
        if not target.is_dir():
            print(f"FEHLER: --specialty-dir '{args.specialty_dir}' existiert nicht oder ist kein Ordner.")
            sys.exit(1)
        os.chdir(target)
        print(f"Fachgebiets-Verzeichnis gewechselt zu: {target.resolve()}")

    # Referenz-Dateien scannen
    ref_files = load_reference_files(args.refs_dir)
    print(f"Gefundene Referenzen in {args.refs_dir}: {ref_files if ref_files else 'Keine'}")
    ...
    out_dir = Path(f"output/{project_name}_{timestamp}")
